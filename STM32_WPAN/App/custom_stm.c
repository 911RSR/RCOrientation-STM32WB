...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomOrientationHdle;                    /**< Orientation handle */
  uint16_t  CustomOrientation_CommandHdle;                  /**< Orientation_command handle */
  uint16_t  CustomServo_FeedbackHdle;                  /**< Servo_feedback handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeOrientation_Command = 8;
uint16_t SizeServo_Feedback = 10;
...
#define COPY_ORIENTATION_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x7b,0x5d,0x00,0x00,0x6a,0x2e,0x4d,0x6b,0x9e,0x6f,0x8d,0x2b,0x7c,0x5a,0x11,0x00)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomServo_FeedbackHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_SERVO_FEEDBACK_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_SERVO_FEEDBACK_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomOrientation_CommandHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
          ...
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomOrientation_CommandHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for Orientation +
   *                                2 for Orientation_command +
   *                                2 for Servo_feedback +
   *                                1 for Servo_feedback configuration descriptor +
   *                              = 6
   *
  ...
  COPY_ORIENTATION_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomOrientationHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: Orientation, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: Orientation , handle = 0x%04x \n\r", CustomContext.CustomOrientationHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomOrientationHdle,
                          UUID_TYPE_16, &uuid,
                          ...
                          &(CustomContext.CustomOrientation_CommandHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : ORIENTATION_COMMAND, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : ORIENTATION_COMMAND , handle = 0x%04x \n\r", CustomContext.CustomOrientation_CommandHdle);
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomOrientationHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomServo_FeedbackHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : SERVO_FEEDBACK, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : SERVO_FEEDBACK , handle = 0x%04x \n\r", CustomContext.CustomServo_FeedbackHdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_ORIENTATION_COMMAND:
      ret = aci_gatt_update_char_value(CustomContext.CustomOrientationHdle,
                                       CustomContext.CustomOrientation_CommandHdle,
                                       0, /* charValOffset */
                                       SizeOrientation_Command, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value ORIENTATION_COMMAND command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value ORIENTATION_COMMAND command\n\r");
      }
    ...
    case CUSTOM_STM_SERVO_FEEDBACK:
      ret = aci_gatt_update_char_value(CustomContext.CustomOrientationHdle,
                                       CustomContext.CustomServo_FeedbackHdle,
                                       0, /* charValOffset */
                                       SizeServo_Feedback, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value SERVO_FEEDBACK command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value SERVO_FEEDBACK command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_ORIENTATION_COMMAND:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomOrientationHdle, CustomContext.CustomOrientation_CommandHdle, SizeOrientation_Command, pPayload);
    ...
    case CUSTOM_STM_SERVO_FEEDBACK:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomOrientationHdle, CustomContext.CustomServo_FeedbackHdle, SizeServo_Feedback, pPayload);
